...
/**
 * memory_add - Add @page free pages to @m
 */
void memory_add(struct memory *m, uint64_t page)
{
	m->free_pages += page;
}

/**
 * memory_remove - Remove @page free pages from @m
 */
void memory_remove(struct memory *m, uint64_t page)
{
	assert(m->free_pages >= page);
	m->free_pages -= page;
}
